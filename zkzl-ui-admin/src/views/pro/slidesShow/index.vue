      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['pro:slides-show:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['pro:slides-show:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getSlidesShowPage(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      this.reset();
      const id = row.id;
      getSlidesShow(id).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updateSlidesShow(this.form).then(response => {
        ...
        // 添加的提交
        createSlidesShow(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除轮播图编号为"' + id + '"的数据项?').then(function() {
          return deleteSlidesShow(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有轮播图数据项?').then(() => {
          ...
          return exportSlidesShowExcel(params);
        }).then(response => {
          this.$download.excel(response, '轮播图.xls');
